...
// 配置参数
        $config = array(
            ...
            'Proxy' => '',
            'SecretId' => SystemModel::config("txy_cos_secret_id"), // 固定密钥
            'SecretKey' => SystemModel::config("txy_cos_secret_key"), // 固定密钥
            'Bucket' => SystemModel::config("txy_cos_secret_bucket"),
            'Region' => SystemModel::config("txy_cos_secret_region"),
            'AllowPrefix' => '*', // 这里改成允许的路径前缀，这里可以根据自己网站的用户登录态判断允许上传的目录，例子：* 或者 a/* 或者 a.jpg
        );
        Log::writeRemote($config);
...
// 获取临时密钥
        function getTempKeys($config)
        {
            ...
            $params['Signature'] = urlencode(getSignature($params, $config['SecretKey'], $Method, $config));
