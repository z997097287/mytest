...
class File
{
     ...
     * @param bool $path
     * @param bool $log_file_name
     * @return bool
     */
    public function save(array $log = [], $path = false, $log_file_name = false)
    {
        $cli = IS_CLI ? '_cli' : '';
        if (empty($path)) {
        ...
        if (empty($log_file_name)) {
            $log_file_name = date('Ym') . DS . date('d') . $cli;
        }
        $destination = $path . $log_file_name . '.log';
        ...
        foreach ($log as $type => $val) {
            ...
            if (in_array($type, $this->config['apart_level'])) {
                // 独立记录的日志级别
                $filename = $this->config['path'] . DS . date('d') . '_' . $type . $cli . '.log';
